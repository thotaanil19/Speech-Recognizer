 ...
 * <p>
 * A {@link edu.lu.sphinx.frontend.DataStartSignal DataStartSignal} will be
 * placed before the start of the first file, and a
 * {@link edu.lu.sphinx.frontend.DataEndSignal DataEndSignal} after the last
...
public class ConcatAudioFileDataSource extends AudioFileDataSource implements
    ...
    class InputStreamEnumeration implements Enumeration<AudioInputStream> {
        ...
        public AudioInputStream nextElement() {
            ...
            if (nextFile != null) {
                try {
                    ...
                    stream = ais;
                    logger.debug("Strating processing of '"
                            + lastFile.getFile() + '\'');
        ...
        public URL readNext() {
            if (lastFile != null) {
                logger.debug("Finished processing of '" + lastFile.getFile()
                        + '\'');
                for (AudioFileProcessListener fl : fileListeners)
